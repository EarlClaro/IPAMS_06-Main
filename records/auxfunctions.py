...
def newRecordAdded(request, userID, adviserID, recordID):
	try:
		...
		redirect_path = base_url[0] + 'record/evaluation/' + str(record.pk)
...
def resubmission(request, userID, recordID, checkedByID):
	...
	recipient = User.objects.get(pk=checkedByID)
...
def send_email_to_recipient(subject, message, from_email, recipient_email):
	...
	try:
		...
		if status == 'approved':
			if user.role.name == 'Adviser':
				...
				threads = []
				for account in ktto_accounts:
					send_thread = Thread(target=send_email_to_recipient,
										 args=(mail_subject, message, settings.EMAIL_HOST_USER, account.email))
					send_thread.start()
...
def deleteRecord(request, userID, recordID, reason): #Only the admins can approve to delete a record
	...
	redirect_path = base_url[0] + 'records/evaluation/'
