...
from records.models import Record, Publication, Conference, Author, Collaboration, Budget, AuthorRole, PublicationLevel, \
    Classification, PSCEDClassification, CheckedRecord, RecordUpload, RecordAuthenticationPin, CheckedRecordComment
...
class CheckedRecordCommentForm(forms.ModelForm):
    use_required_attribute = False

    class Meta:
        model = CheckedRecordComment
        fields = ('comment',)
        labels = {
...
class RecordAuthenticationPinForm(forms.ModelForm):

    class Meta:
        model = RecordAuthenticationPin
        exclude = ['record', 'user', 'role']