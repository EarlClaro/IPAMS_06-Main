    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            if 'all' in request.POST:
                ...
                notifications = Notification.objects.filter(Q(recipient=user.id) & Q(is_read=True)).order_by(
                    '-date_created')
                return render(request, self.name, filterHelperStudent(request, notifications, 'Read'))
            elif 'unread' in request.POST:
                notifications = Notification.objects.filter(Q(recipient=user.id) & Q(is_read=False)).order_by(
                    '-date_created')
                return render(request, self.name, filterHelperStudent(request, notifications, 'Unread'))
            elif 'approved' in request.POST:
                notifications = Notification.objects.filter(
                    Q(recipient=user.id) & Q(notif_type=NotificationType.objects.get(pk=8))).order_by('-date_created')
                return render(request, self.name, filterHelperStudent(request, notifications, 'Approved'))
            elif 'declined' in request.POST:
                notifications = Notification.objects.filter(
                    Q(recipient=user.id) & Q(notif_type=NotificationType.objects.get(pk=9))).order_by('-date_created')
            ...
            if request.POST.get('displayContent'):
                ...
                elif notif.notif_type.name == 'Approved Request to Download Abstract':
